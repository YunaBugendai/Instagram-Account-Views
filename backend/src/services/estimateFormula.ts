...
/**
 * Bugüne ve kullanıcı adına göre deterministik bir "günlük varyasyon" üretir.
 * Aynı gün içinde aynı hesap için tekrar hesaplanırsa hep aynı sonucu verir,
 * gün değiştiğinde doğal biçimde değişir. Rastgele değil, tekrarlanabilir.
 */
function dailyJitter(username: string): number {
  ...
  // hash'in ilk 4 baytını -1..1 aralığına normalize et
  const signedInt = hash.readInt32BE(0);
  const normalized = signedInt / 2147483648; // -1..1
  return normalized * 0.04; // ±%4
}
 ...
 * üretilen, eğlence amaçlı bir tahmindir. Sonuç ekranında bu açıkça belirtilmelidir.
 */
